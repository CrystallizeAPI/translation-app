...
import { Language, Preferences } from "../use-cases/contracts/types";
...
export const createTranslator = async ({ apiKey }: Deps) => {
    ...
    const translate = async ({ text, language, preferences }: TranslatorArgs) => {
        ...
        const messages: ChatCompletionMessage[] = [
            {
                role: "user",
                content: `Translate from ${from} to ${to} (delimited with XML tags). ${preferences.customPromptFromUser ? `And ${preferences.customPromptFromUser}`
                    : ""
